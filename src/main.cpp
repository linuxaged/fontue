...
class MsdfGen
{
  public:
    ...
    void Gernerate( Bitmap<FloatRGB>& bitmap, Shape& shape )
    {
        generateMSDF( bitmap, shape, 4.0, 1.0, Vector2( 4.0, 4.0 ) );
    }
...
inline int32_t ftoi( const double d ) { return (int32_t)d; }

inline int32_t dtoi( const double d ) { return (int32_t)d; }
...
static bool PlaceGlyphs( std::vector<CGlyph*> const& glyphs, int32_t const sideX,
                         int32_t const sideY, std::vector<CRectangle*>& usedRects )
{
        ...
        if ( !emptyRect->CanFitGlyph( g ) )
        {
            ...
            for ( ; i >= 0; --i )
            {
                ...
                if ( rect->CanFitGlyph( g ) )
                {
                    ...
                    emptyRect     = rect;
                    std::sort( emptyRects.begin(), emptyRects.end(),
                               CRectangle::CompareGlyphHeight );
                    break;
                }
            }
            if ( i < 0 )
...
bool AddCodePointsFromFile( char const* charFile, std::vector<uint32_t>& codePoints,
                            bool const verifyAscii, std::string& asciiErrors,
...
static int CreateBitmapFont( int const argc, char const* argv[], FT_Library& ftLibrary )
{
    ...
    for ( int i = 3; i < argc; ++i )
    {
        ...
        else if ( strcasecmp( argv[i], "-cf" ) == 0 || strcasecmp( argv[i], "-charFile" ) == 0 )
        {
            ...
            if ( !AddCodePointsFromFile( charFile.c_str(), glyphToUnicodeMap, verifyAscii,
                                         asciiErrors, nonAsciiChars ) )
            {
    ...
    if ( faceIdx >= 0 )
    {
        ...
        {
            FT_Select_Size( fallbackFontFaces[i],
                            0 ); // cannot specify a face index for fallback fonts
        }
    ...
    else
    {
        ...
        if ( error )
        {
            ...
            {
                std::cout
                    << "Font is bitmap-only. Use -faceIdx to select a size. Available sizes are:\n";
                for ( int i = 0; i < ftFace->num_fixed_sizes; ++i )
                {
                    FT_Bitmap_Size& size = ftFace->available_sizes[i];
                    std::cout << "Index: " << i << ", size: " << size.size
...

    {
        ...
        char buffer[1024];
        sprintf( buffer, "Unicode code points for '%s':\nTotal code points: %i\n",
                 outFileName.c_str(), static_cast<int>( glyphToUnicodeMap.size() ) );
    ...
    for ( int32_t i = 0; i < static_cast<int32_t>( glyphToUnicodeMap.size() ); ++i )
    {
        ...
        if ( ftBitmap.pixel_mode == FT_PIXEL_MODE_GRAY )
        {
            ...
            {
                std::cout << "Wrong bitmap format (!256 grays) for glyph character code "
                          << charCode << ".\n";
                continue;
            }
            ...
            for ( int y = 0; y < ftBitmap.rows; ++y )
            {
                for ( int x = 0; x < ftBitmap.width; ++x )
                {
                    ...
                    g.Bitmap->SetPixelRGBA( x + ( hpad >> 1 ), y + ( vpad >> 1 ), gray, gray, gray,
                                            alpha );
                }
            }
            if ( grid )
    ...
    if ( signedDistanceField )
    {
        ...
        for ( int32_t y = 0; y < sdfBitmap->Height; ++y )
        {
            ...
            for ( int32_t x = 0; x < sdfBitmap->Width; ++x )
            {
                ...
                sdfBitmap->SetPixelRGBA( x, y, b, b, b,
                                         alphaValue < 0 ? b : static_cast<uint8_t>( alphaValue ) );
            }
        }

        std::cout << "done.            \n"; // extra spaces are intentional to clear previous output

        std::cout << "Writing " << targetWidth << "x" << actualHeight << " image file '"
                  << sdfFileName << "'...\n";
